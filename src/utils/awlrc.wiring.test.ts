...
test('逐字进度改用本地时钟 + 段边界定时器，不再按固定间隔轮询位置', () => {
  ...
  assert.match(hook, /isFinished/, '缺少「整行唱完」的信号，翻译/罗马音无法等扫完再淡入')
  for (const file of ['../screens/PlayDetail/Vertical/Lyric.tsx', '../screens/PlayDetail/Horizontal/Lyric.tsx']) {
    const src = read(file)
    assert.match(src, /extendedOpacity/, file + ' 的翻译/罗马音没有等扫完再淡入')
    assert.match(src, /onFinished=/, file + ' 没有接收「唱完」回调')
  }
